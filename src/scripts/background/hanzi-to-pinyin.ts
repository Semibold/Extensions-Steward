...
function hasChinaCollator() {
    ...
    if (hasChinaCollator) {
        ...
        if (DEBUG) {
            console.log(TAG, `Self validation. Result: ${doSelfValidation()}`);
        }
    } else {
        console.warn(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
    }
    return hasChinaCollator;
}
...
function doSelfValidation() {
    ...
    for (const c of UNIHANS) {
        ...
        const curString = c.toString();
        const cmp = COLLATOR.compare(lastString, curString);
        if (cmp >= 0) {
            console.error(
                TAG,
...
function getToken(character: string) {
    ...
    if (offset < 0) {
        ...
        while (begin <= end) {
            ...
            const unihan = UNIHANS[offset].toString();
    ...
    const pinyin = new StringBuilder();
    for (let j = 0; j < PINYINS[offset].length && PINYINS[offset][j] !== 0; j++) {
        pinyin.append(String.fromCodePoint(PINYINS[offset][j]));
    }
